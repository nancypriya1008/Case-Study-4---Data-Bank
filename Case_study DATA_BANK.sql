...
VALUES
  ...
  select r.region_name,n.region_id,count(node_id)
  from regions r left join customer_nodes n on r.region_id = n.region_id
  group by 1,2;
 ...
 where end_date !='9999-12-31';
  ...
  select txn_type,count(txn_type) as unique_count,
sum(txn_amount) from customer_transactions
group by 1;

--2 What is the average total historical deposit counts and amounts for all customers?
with count_tab as(
select customer_id,txn_type,sum(txn_amount)as total_amount,
count(txn_type) as Total_Count from customer_transactions
where txn_type = "deposit"
group by customer_id
order by 1
)
select round(avg(total_count),0)as count,round(avg (total_amount),0)as amount
from count_tab;
...
sum(case when txn_type != "deposit" then 1 else 0 end) as purchase_count
from customer_transactions
group by customer_id, month(txn_date)
)
select mnth ,count(customer_id) as customer
from txn_count
where deposite_count >1 and purchase_count =1
group by mnth;
